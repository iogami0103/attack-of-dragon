...
inline UINT JustAudioWindowsDispatchMessage() {
  static const UINT message =
      RegisterWindowMessageW(L"just_audio_windows_dispatch_message");
  return message;
}
...
public:
  ...
  JustAudioEventSink::JustAudioEventSink(flutter::BinaryMessenger* messenger, HWND platform_hwnd, const std::string& id)
    : platform_hwnd_(platform_hwnd), platform_thread_id_(GetCurrentThreadId()) {
    auto event_channel =
    ...
    auto event_handler = std::make_unique<flutter::StreamHandlerFunctions<>>(
      ...
      if (self->closed_) return nullptr;
  ...
  void Success(const EncodableValue& event) {
    Dispatch([event](JustAudioEventSink* self) {
      if (self->sink) {
  ...
  void Error(const std::string& error_code,
    ...
    Dispatch([error_code, error_message](JustAudioEventSink* self) {
      if (self->sink) {
...
private:
  void Dispatch(std::function<void(JustAudioEventSink*)> action) {
    if (closed_) return;
    ...
    if (platform_hwnd_ == nullptr) {
      std::cerr << "[just_audio_windows] Dispatch dropped: no platform hwnd" << std::endl;
      return;
    }

    auto self = shared_from_this();
    auto task = new std::function<void()>(
      [self, action = std::move(action)]() mutable {
        if (!self->closed_) {
          action(self.get());
        }
      });
    if (!PostMessageW(
      platform_hwnd_,
      JustAudioWindowsDispatchMessage(),
      0,
      reinterpret_cast<LPARAM>(task))) {
      std::cerr << "[just_audio_windows] Dispatch dropped: PostMessage failed, error=" << GetLastError() << std::endl;
      delete task;
    }
  }
...
private:
  ...
  void Dispose() {
    ...
    if (event_sink_) event_sink_->Close();
    if (data_sink_) data_sink_->Close();
    event_sink_.reset();
    data_sink_.reset();
    mediaPlayer.Close();
  }
public:
  ...
  std::shared_ptr<JustAudioEventSink> event_sink_ = nullptr;
  std::shared_ptr<JustAudioEventSink> data_sink_ = nullptr;
...
public:
  AudioPlayer::AudioPlayer(std::string idx, flutter::BinaryMessenger* messenger, HWND platform_hwnd) {
    id = idx;
    ...
    event_sink_ = std::make_shared<JustAudioEventSink>(messenger, platform_hwnd, "com.ryanheise.just_audio.events." + idx);
    data_sink_ = std::make_shared<JustAudioEventSink>(messenger, platform_hwnd, "com.ryanheise.just_audio.data." + idx);
    ...
    // Media opened event. Reports the natural duration once the source has
    // been opened, even when playback has not started yet.
    media_opened_token_ = mediaPlayer.MediaOpened([this](auto, const auto& args) -> void {
      if (disposed_) return;
      broadcastState();
    });
  ...
  void AudioPlayer::HandleMethodCall(
    ...
    if (method_call.method_name().compare("load") == 0) {
      ...
      // just_audio >= 0.10 waits for a playback event whose processing state
      // is no longer "loading" before completing load(). The media player does
      // not emit PlaybackStateChanged until playback starts, so broadcast the
      // current state here to let the Dart side proceed.
      broadcastState();
  ...
  */
  ...
  void AudioPlayer::broadcastPlaybackEvent() {
    ...
    {
      // The Windows playback session can reject BufferingProgress during
      // startup. Treat local assets as fully buffered instead of logging noise.
      bufferingProgress = 1;
    }
