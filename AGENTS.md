...
On macOS, always prefix shell commands with the Mac RTK executable path:

```bash
/opt/homebrew/bin/rtk git status
/opt/homebrew/bin/rtk flutter analyze
/opt/homebrew/bin/rtk bash tools/github_workflow_check.sh
```

Do not try to use the Windows RTK path on Mac.

## GitHub workflow

- Remote: `origin` -> `https://github.com/iogami0103/attack-of-dragon.git`
- Before editing, run `git fetch --prune origin`, `git status --short --branch`, and `git pull --ff-only` through the platform RTK path.
...
- Before pushing, run `powershell -ExecutionPolicy Bypass -File tools\github_workflow_check.ps1 -RunFlutterChecks` on Windows or `bash tools/github_workflow_check.sh --run-flutter-checks` on macOS, through the platform RTK path.
...
```bash
/opt/homebrew/bin/rtk flutter analyze
/opt/homebrew/bin/rtk flutter test
```
...
```bash
/opt/homebrew/bin/rtk flutter build apk --debug
```
